...
class ResPartner(models.Model):
    ...
    def button_update_dnic_values(self):
        self.with_context(call_from_button=True).update_dnic_values()
    ...
    def get_cv_main_values(self, response):
        ...
        if self._context.get('call_from_button'):
    ...
    def write(self, values):
        if values is None:
            values = {}
        # Se agrega el código para habilitar el calculo de los campos de ID digital al guardar el formulario
        # Esto posibilita realizar pruebas a la lógica
        if not self._context.get('call_from_button', False) \
                and list(set(DNIC_FROZEN_COLUMNS).intersection(set(values.keys()))):
            values.update(self.get_cv_main_values(values))
