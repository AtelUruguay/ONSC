...
class ONSCMigrations(models.Model):
    ...
    def _v27(self, cv_ids=False):
        ...
        BFormation = self.env['onsc.cv.basic.formation'].sudo().with_context(ignore_base_restrict=True, ignore_documentary_status = True)
        AFormation = self.env['onsc.cv.advanced.formation'].sudo().with_context(ignore_base_restrict=True, ignore_documentary_status = True)
        Course = self.env['onsc.cv.course.certificate'].sudo().with_context(ignore_base_restrict=True, ignore_documentary_status = True)
        ...
        ParticipationEvent = self.env['onsc.cv.participation.event'].sudo().with_context(ignore_base_restrict=True, ignore_documentary_status = True)
        ...
        TutoringOrientationSupervision = self.env['onsc.cv.tutoring.orientation.supervision'].sudo().with_context(
            ignore_base_restrict=True, ignore_documentary_status = True)
        Voluntariado = self.env['onsc.cv.volunteering'].sudo().with_context(ignore_base_restrict=True, ignore_documentary_status = True)
        WorkExperience = self.env['onsc.cv.work.experience'].sudo().with_context(ignore_base_restrict=True, ignore_documentary_status = True)
        WorkInvestigation = self.env['onsc.cv.work.investigation'].sudo().with_context(ignore_base_restrict=True, ignore_documentary_status = True)
        WorkTeaching = self.env['onsc.cv.work.teaching'].sudo().with_context(ignore_base_restrict=True, ignore_documentary_status = True)
