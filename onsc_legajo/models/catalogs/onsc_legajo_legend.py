...
class ONSCLegajoLegend(models.Model):
    ...
    order = fields.Integer(string='Órden', compute='_compute_order', store=True)
    active = fields.Boolean("Activo", default=True)

    @api.depends('regime_id', 'descriptor1_id')
    def _compute_order(self):
        for record in self:
            if record.regime_id and record.descriptor1_id:
                record.order = 1
            elif record.regime_id:
                record.order = 2
            elif record.descriptor1_id:
                record.order = 3
            else:
                record.order = 4
    ...
    def _get_legajo_legend(self, regime_id, descriptor1_id):
        ...
        for rec in self.sudo().search([]):
            ...
            if regime_id and match_regime and descriptor1_id and match_descriptor1:
                result_record = rec
            elif regime_id and match_regime and not rec.descriptor1_id and not result_record:
                result_record = rec
            elif descriptor1_id and match_descriptor1 and not rec.regime_id and not result_record:
                result_record = rec
