...
class ONSCLegajoElectoralAct(models.Model):
    ...
    def _check_date_range(self):
        for record in self:
            ...
            if record.date_until_entry_control and record.date_until_consultation_control and \
                    record.date_until_entry_control > record.date_until_consultation_control:
...
class ONSCLegajoRoleElectoralAct(models.Model):
    ...
    _sql_constraints = [
        ('code_unique', 'unique(code)', _(u'Ya existe un Rol acto electoral con ese código')),
        ('name_unique', 'unique(name)', _('Ya existe un Rol acto electoral con ese nombre'))
    ]
