...
class ONSCLegajoAltaVL(models.Model):
    ...
    date_start = fields.Date(string="Fecha de alta",  default=fields.Date.today())
    ...
    inciso_id = fields.Many2one('onsc.catalog.inciso', string='Inciso', default=lambda self: self._get_default_inciso_id())
    inciso_readonly = fields.Boolean(compute="_compute_is_readonly")
    operating_unit_id = fields.Many2one("operating.unit", string="Unidad ejecutora", default=lambda self: self._get_default_ue_id())
    operating_unit_readonly = fields.Boolean(compute="_compute_is_readonly")
    ...
    responsable_uo = fields.Boolean(string="¿Responsable de UO?", related="security_job_id.is_uo_manager")
    ...
    reserva_sgh = fields.Boolean(string="¿Tiene reserva en SGH?")
    regime_id = fields.Many2one('onsc.legajo.regime', string='Régimen')
    es_presupuestado = fields.Boolean(related="regime_id.presupuesto", store=True)
    indVencimiento = fields.Boolean(related="regime_id.indVencimiento", store=True)
    ...
    emergency_service_id = fields.Many2one("onsc.legajo.health.provider", u"Cobertura de salud")
    additional_information_discharge = fields.Text(string='Información adicional')
    ...
    state = fields.Selection(STATES, string='Estado', default='borrador')
    ...
    def _compute_is_readonly(self):
        for rec in self:
            rec.inciso_readonly = self.user_has_groups('onsc_legajo.group_legajo_alta_vl_recursos_humanos_inciso') or \
                self.user_has_groups('onsc_legajo.group_legajo_alta_vl_recursos_humanos_ue')
            rec.operating_unit_readonly = self.user_has_groups('onsc_legajo.group_legajo_alta_vl_recursos_humanos_ue')
