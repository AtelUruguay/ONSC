...
class OrganizationalChart(models.Model):
    ...
    def get_employee_data(self, emp_id):
        parent_emp = self.env['hr.department'].search([('id', '=', str(emp_id))])
        data = {
        ...
        employees = self.env['hr.department'].search([('parent_id', '=', parent_emp.id)])
    ...
    def get_children(self, emp, style=False):
        data = []
        emp_data = {'name': emp.name, 'title': "Hola", 'office': self._get_image(emp)}
